document.addEventListener("DOMContentLoaded",()=>{let e=document.querySelectorAll("pre"),t=document.getElementById("toggleMode"),$=document.body,a=e=>{let t=[{type:"Composer Command",pattern:/composer require/},{type:"Environment Variables",pattern:/^[A-Z_]+="[^"]*"$/gm},{type:"Artisan Command",pattern:/php artisan/},{type:"PHP Code",pattern:/<\?php/},{type:"HTML Code",pattern:/<!DOCTYPE html>/i},{type:"Blade Component",pattern:/<x-[\w-]+/}];for(let $ of t)if($.pattern.test(e))return $.type;return"Code Snippet"};e.forEach((e,t)=>{let $=document.createElement("div");$.className="cb-code-container";let l=document.createElement("div");l.className="cb-code-header";let n=document.createElement("div");n.className="cb-code-title";let o=a(e.textContent);n.textContent=o,"PHP Code"===o&&$.classList.add("cb-php-code");let d=document.createElement("button");d.className="cb-copy-btn",d.title="Copy to clipboard",d.innerHTML=`
